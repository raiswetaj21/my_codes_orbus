...
def create_line_chart(data, height=250):
    fig = px.line(data, x='Date', y='Value', title='Line Chart')
    fig.update_layout(margin=dict(l=20, r=20, t=30, b=20), height=height)
    return fig


def create_area_chart(data, height=250):
    fig = px.area(data, x=data.index, y=data.columns, title='Area Chart')
    fig.update_layout(margin=dict(l=20, r=20, t=30, b=20), height=height)
    return fig


def create_pie_chart(labels, values, height=250):
    fig = px.pie(names=labels, values=values, title='Pie Chart')
    fig.update_layout(margin=dict(l=20, r=20, t=30, b=20), height=height)
    return fig


def create_exploded_donut_chart(labels, values, height=250):
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=0.4, pull=[0.1, 0.1, 0.1])])
    fig.update_layout(title_text="Exploded Donut Chart", margin=dict(l=20, r=20, t=30, b=20), height=height)
    return fig


def create_bar_chart(categories, values, height=250):
    fig = px.bar(x=categories, y=values, title='Bar Chart')
    fig.update_layout(margin=dict(l=20, r=20, t=30, b=20), height=height)
    return fig


def create_sunburst_chart(data, height=250):
