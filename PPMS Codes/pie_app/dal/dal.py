import pandas as pd
from pie_app.db_config.db_config import get_db_connection
import warnings
...
def get_data_from_db(table_name, x_column, y_column, date=None):
    ...
    try:
        ...
        query = f"SELECT {x_column}, {y_column}, ProjectStartDate FROM {table_name}"
        if date:
            query += f" WHERE ProjectStartDate = '{date}'"
