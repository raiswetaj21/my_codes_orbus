...
common_query_part = """
SELECT TOP 10 Vehiclename, WARD, FEEDERNAME, STATUS, First_Intersection_Time, Last_Intersection_Time, EXPECTEDTIME
...
SELECT Vehiclename, WARD, FEEDERNAME, STATUS, First_Intersection_Time, Last_Intersection_Time, EXPECTEDTIME
FROM FEEDER_BASE_DP_AT
WHERE CAST(DATE_CLEAN_NEW AS DATE) = CAST(GETDATE() AS DATE)
AND WARD = '{}'
AND STATUS != 'COVERED'
...
for ward in ward_names:
    ...
    subject = f"Daily Feeder Report Email from SCS for {ward}"
    message = f"""
    <html>
    <body>
    <p>Dear Recipient,</p>
    <p>Please find snippet of feeder details from {ward} and a detailed spreadsheet of uncovered and unattended feeder vehicles in the attachment:</p>
    {dataframe_html}
